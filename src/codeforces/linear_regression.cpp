...
public:
    ...
    vector<matrix_> X_batch;
    vector<vector_> y_batch;

    void divide_to_mininatches();
...
void Dataset::divide_to_mininatches() {
    for (int batch = 0; batch < n; batch += minibatch_size) {
        X_batch.emplace_back(X.begin() + batch, X.begin() + min(batch + minibatch_size, n));
        y_batch.emplace_back(y.begin() + batch, y.begin() + min(batch + minibatch_size, n));
        batch_n++;
    }
}
...
int main() {
    ...
    LinearRegression lr;
    vector_ coef = lr.fit(df, 550000, 0.3);
    for (double c : coef) {
        printf("%lf\n", c);
    }
    return 0;
